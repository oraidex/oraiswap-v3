...
fn test_swap_x_to_y() {
    ...
    assert_eq!(upper_tick.fee_growth_outside_x, FeeGrowth::new(U256::from(0)));
    assert_eq!(
        middle_tick.fee_growth_outside_x,
        FeeGrowth::new(U256::from(30000000000000000000000))
    );
    assert_eq!(lower_tick.fee_growth_outside_x, FeeGrowth::new(U256::from(0)));
...
fn test_swap_y_to_x() {
    ...
    assert_eq!(middle_tick.liquidity_change, liquidity_delta);
    assert_eq!(upper_tick.liquidity_change, liquidity_delta);
    assert_eq!(upper_tick.fee_growth_outside_y, FeeGrowth::new(U256::from(0)));
    assert_eq!(
        middle_tick.fee_growth_outside_y,
        FeeGrowth::new(U256::from(30000000000000000000000))
    );
    assert_eq!(lower_tick.fee_growth_outside_y, FeeGrowth::new(U256::from(0)));
    assert!(lower_tick_bit);
    assert!(middle_tick_bit);
    assert!(upper_tick_bit);
}
