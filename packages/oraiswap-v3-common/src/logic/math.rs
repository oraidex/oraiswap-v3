...
pub fn get_liquidity_by_x_sqrt_price(
    ...
    if upper_sqrt_price < current_sqrt_price {
        return Err(ContractError::UpperSqrtPriceLess);
    }
...
pub fn get_liquidity_by_y_sqrt_price(
    ...
    if current_sqrt_price < lower_sqrt_price {
        return Err(ContractError::CurrentSqrtPriceLess);
    }
...
mod tests {
    ...
    fn get_liquidity_by_x_test() {
        ...
        // below current tick
        {
            let lower_tick = -50;
            let upper_tick = 10;
            let err = get_liquidity_by_x(x, lower_tick, upper_tick, current_sqrt_price, true)
                .unwrap_err();
            assert!(matches!(err, ContractError::UpperSqrtPriceLess));
        }
        // in current tick
        {
