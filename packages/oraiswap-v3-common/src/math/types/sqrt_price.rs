...
pub fn check_tick_to_sqrt_price_relationship(
    ...
    if tick_index.checked_add(tick_spacing as i32).unwrap() > MAX_TICK {
        ...
        let upper_bound =
            SqrtPrice::from_tick(tick_index.checked_add(tick_spacing as i32).unwrap())?;
...
pub fn calculate_sqrt_price(tick_index: i32) -> Result<SqrtPrice, ContractError> {
    // checking if tick be converted to sqrt_price (overflows if more)
    let tick = tick_index.abs();

    if tick > MAX_TICK {
        return Err(ContractError::TickOverBounds);
    }
    ...
    Ok(if tick_index >= 0 {
        ...
        SqrtPrice::new(u128::uint_cast(
            SqrtPrice::one()
                ...
                .ok_or(ContractError::CheckedDiv)?,
        ))
    })
}
